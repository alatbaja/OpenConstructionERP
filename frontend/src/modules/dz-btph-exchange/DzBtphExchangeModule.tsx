...
import { DZ_BTPH_TEMPLATE, LOTS_BTPH } from './dzBtphTemplate';

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

type DzExportFormat = 'BPU' | 'DQE';
    ...
    <div className="border border-border-light rounded-lg overflow-hidden">
      <div className="px-3 py-2 bg-surface-tertiary/50 flex items-center justify-between">
        <span className="text-xs font-medium text-content-secondary">
          {t('dz_btph.preview', { defaultValue: 'Preview' })}: {positions.length}{' '}
          {t('dz_btph.positions', { defaultValue: 'positions' })}
        </span>
        {positions.length > 20 && (
          <button
          ...
          >
            {showAll
              ? t('dz_btph.show_less', { defaultValue: 'Show less' })
              : t('dz_btph.show_all', { defaultValue: `Show all ${positions.length}` })}
          </button>
        )}
      </div>
          ...
          <thead>
            <tr className="bg-surface-secondary/50 sticky top-0">
              ...
              <th className="px-3 py-1.5 text-left font-medium text-content-secondary w-32">
                {t('dz_btph.lot', { defaultValue: 'Lot' })}
              </th>
            </tr>
          </thead>
  ...
  return (
    <div className="rounded-lg border border-border-light bg-surface-secondary/30 p-3">
      ...
      >
        ...
        {t('dz_btph.lot', { defaultValue: 'Lot' })}s BTPH ({LOTS_BTPH.length}) —{' '}
        <span className="text-2xs text-content-quaternary">8 Lots officiels (Décret 15-247)</span>
        <span className="ml-auto text-2xs text-content-quaternary">{expanded ? '−' : '+'}</span>
      </button>
      {expanded && (
        <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-1">
          {LOTS_BTPH.map((lot) => (
            <div key={lot.code} className="flex items-center gap-2 text-2xs py-0.5">
              <Badge variant="neutral" className="font-mono text-2xs w-8 text-center shrink-0">
                {lot.code}
              </Badge>
              <span className="text-content-secondary truncate">
                {isAr ? lot.labelAr : isFr ? lot.labelFr : lot.label}
              </span>
            </div>
          ))}
        </div>
...
export default function DzBtphExchangeModule() {
  ...
  const [exportFormat, setExportFormat] = useState<DzExportFormat>('DQE');
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      try {
        const result = await parseExcelFile(file, DZ_BTPH_TEMPLATE.defaultColumns);

        if (result.positions.length === 0) {
          setParseError(
            t('dz_btph.parse_error', {
              defaultValue:
                'No positions found. Ensure the file is a valid DZ BPU/DQE spreadsheet (CSV or Excel).',
            }),
          );
        } else {
          ...
          addToast({
            type: 'success',
            title: t('dz_btph.parsed_ok', { defaultValue: 'File parsed successfully' }),
            message: `${result.positions.length} positions found`,
          });
        }
      } catch {
        setParseError(
          t('dz_btph.parse_error_generic', {
            defaultValue: 'Failed to parse the BPU/DQE file.',
          }),
        );
      }
    },
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      addToast({
        type: 'success',
        title: t('dz_btph.import_complete', { defaultValue: 'BPU/DQE import complete' }),
      ...
      addToast({
        type: 'error',
        title: t('dz_btph.import_failed', { defaultValue: 'BPU/DQE import failed' }),
        message: errorMsg,
      });
    } finally {
  ...
  const handleExport = useCallback(() => {
    if (exportablePositions.length === 0) {
      addToast({
        type: 'warning',
        title: t('dz_btph.no_positions', { defaultValue: 'No positions to export' }),
      });
      return;
    }
    setIsExporting(true);
    try {
      ...
      const suffix = exportFormat === 'BPU' ? 'BPU' : 'DQE';
      const filename = `${safeName}_${suffix}_DZ.csv`;

      const result = exportToCSV(exportablePositions, DZ_BTPH_TEMPLATE, filename, {
      ...
      addToast({
        type: 'success',
        title: t('dz_btph.export_complete', { defaultValue: 'BPU/DQE export complete' }),
        message: `${result.positionCount} positions exported to ${result.filename}`,
      });
    } catch (err) {
      addToast({
        type: 'error',
        title: t('dz_btph.export_failed', { defaultValue: 'BPU/DQE export failed' }),
  ...
  const handlePrint = useCallback(() => {
    ...
    printBOQReport(exportablePositions, DZ_BTPH_TEMPLATE, {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-content-primary">
            {t('dz_btph.title', { defaultValue: 'Algérie BTPH — BPU / DQE Import / Export' })}
          </h1>
          <p className="text-sm text-content-tertiary">
            {t('dz_btph.subtitle', {
              defaultValue:
                'Exchange BOQ in Algerian BPU/DQE format (8 Lots BTPH, DZD, TVA LF2026 — Décret 15-247)',
            })}
          </p>
          <p className="text-2xs text-content-quaternary mt-0.5">
            {t('dz_btph.validation_hint', {
              defaultValue:
                'DZ BTPH validation (dz_btph) checks TVA 19%/9%, TAP suppression, Lot 01-08 and DZD currency.',
            })}
          </p>
        </div>
        <Badge variant="blue" className="ml-auto hidden sm:inline-flex">
          DZD · TVA LF2026 · 8 Lots
        </Badge>
      </div>
      ...
      <div className="flex gap-1 border-b border-border">
        ...
        >
          <Upload size={15} />
          {t('dz_btph.tab_import', { defaultValue: 'Import' })}
        </button>
        <button
        ...
        >
          <Download size={15} />
          {t('dz_btph.tab_export', { defaultValue: 'Export' })}
        </button>
      </div>
        ...
        <div className="space-y-5">
          ...
          >
            {importFile ? (
              <div className="space-y-3">
                ...
                {parsedPositions && (
                  <div className="flex items-center justify-center gap-1.5 text-xs text-emerald-600">
                    ...
                    {t('dz_btph.positions_found', { defaultValue: 'positions found' })}
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-content-secondary">
                  {t('dz_btph.drop_file', {
                    defaultValue: 'Drop a BPU/DQE file here (Excel or CSV), or',
                  })}
                </p>
                <Button
                  variant="secondary"
                ...
                >
                  {t('dz_btph.browse', { defaultValue: 'Browse files' })}
                </Button>
                <p className="text-2xs text-content-quaternary">
                  {t('dz_btph.formats_hint', {
                    defaultValue: 'Supported: .csv, .tsv, .xlsx (DZ BPU/DQE formatted BOQ)',
                  })}
                </p>
              </div>
            )}
            <input
              ref={fileInputRef}
              type="file"
              accept={DZ_BTPH_TEMPLATE.acceptedExtensions.join(',')}
              className="hidden"
          ...
          {parsedPositions && parsedPositions.length > 0 && (
            <div className="rounded-xl border border-border bg-surface-primary p-5">
              <h3 className="text-sm font-semibold text-content-primary mb-3">
                {t('dz_btph.target_boq', { defaultValue: 'Import Target' })}
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  >
                    <option value="">
                      — {t('dz_btph.select_boq', { defaultValue: 'Select BOQ' })} —
                    </option>
                    {importBoqs.map((b) => (
                ...
                <div className="flex items-end">
                  ...
                  >
                    {isImporting
                      ? t('dz_btph.importing', { defaultValue: 'Importing...' })
                      : t('dz_btph.import_btn', {
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium">
                ...
                <span className="text-content-primary">
                  ...
                  {t('dz_btph.positions_imported', { defaultValue: 'positions imported' })}
                </span>
              </div>
              {importResult.errors.length > 0 && (
      ...
      {activeTab === 'export' && (
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-border bg-surface-primary p-5">
            <h3 className="text-sm font-semibold text-content-primary mb-3">
              {t('dz_btph.source_boq', { defaultValue: '1. Select BOQ to Export' })}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-medium text-content-tertiary mb-1">
              ...
              <div>
                <label className="block text-xs font-medium text-content-tertiary mb-1">
                  {t('dz_btph.export_format', { defaultValue: 'Format' })}
                </label>
                <select
                  value={exportFormat}
                  onChange={(e) => setExportFormat(e.target.value as DzExportFormat)}
                  className="w-full rounded-lg border border-border bg-surface-secondary px-3 py-2 text-sm"
                >
                  <option value="BPU">
                    {t('dz_btph.bpu_format', { defaultValue: 'BPU (Unit Prices)' })}
                  </option>
                  <option value="DQE">
                    {t('dz_btph.dqe_format', { defaultValue: 'DQE (Quantified)' })}
                  </option>
                </select>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-surface-primary p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text-content-primary">
                  {t('dz_btph.export_summary', { defaultValue: '2. Export Summary' })}
                </h3>
                <button
                ...
                >
                  <Eye size={13} />
                  {showExportPreview
                    ? t('dz_btph.hide_preview', { defaultValue: 'Hide preview' })
                    : t('dz_btph.show_preview', { defaultValue: 'Show preview' })}
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('dz_btph.sections', { defaultValue: 'Lots' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
                    {exportablePositions.filter((p) => p.isSection).length}
                  </div>
                ...
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('dz_btph.format_label', { defaultValue: 'Format' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">{exportFormat}</div>
                </div>
                <div className="rounded-lg bg-surface-secondary/50 p-3 text-center">
                  <div className="text-2xs text-content-tertiary uppercase">
                    {t('dz_btph.prices', { defaultValue: 'Prices' })}
                  </div>
                  <div className="text-lg font-bold text-content-primary">
              ...
              <div className="flex items-center gap-2 text-xs text-content-tertiary">
                <span className="font-mono text-2xs bg-surface-secondary px-1.5 py-0.5 rounded">
                  DZD
                </span>
                TVA LF2026: 19% normal / 9% réduit · Retenue 5% · Avance 15% · CACOBATPH 12,21%
              </div>
              ...
              <div className="flex gap-3">
                ...
                >
                  {t('dz_btph.export_btn', { defaultValue: 'Export as BPU/DQE CSV' })}
                </Button>
                <Button
                  variant="secondary"
                  ...
                  onClick={handlePrint}
                >
                  {t('dz_btph.print_btn', { defaultValue: 'Print PDF' })}
                </Button>
              </div>
            </div>
          )}
      ...
      <div className="flex items-start gap-2 text-xs text-content-quaternary">
        ...
        <p>
          {t('dz_btph.info', {
            defaultValue:
              'BPU (Bordereau des Prix Unitaires) + DQE (Détail Quantitatif Estimatif) are the Algerian BOQ standards per Décret 15-247 (Code des Marchés Publics). Works are organised by 8 Lots BTPH with DZD currency, TVA 19%/9% (LF2026), Retenue 5% and CACOBATPH 12.21%.',
          })}
        </p>
      </div>
    </div>
